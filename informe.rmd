---
title: "Análisis exploratorio SGPOA"
...
Primero que nada, comparamos la cantidad de ANP's en el POA que realizan por objetivo y año (2015, 2016). Escogimos cantidad de proyectos porque, al analizar
los datos, nos dimos cuenta que las ANPs implementan máximo un proyecto por tema
por año, lo que refleja los principales
...
* La cantidad de proyectos realizados es ligeramente mayor en el año 2015 que en
el 2016.
* La mayor cantidad de proyectos registrados en el POA, para ambos años, se refieren
al objetivo de "Conservación y Manejo de la Biodiversidad", al que le siguen, en orden:
...
ggplot(data = Proyectos_objetivo_anio, aes(x = reorder(objetivo, n), y = n,
  ...
  ggtitle("Número de proyectos por año y objetivo") +
  theme(
    axis.text.x = element_blank(),
    plot.title = element_text(hjust = 0.5, face="bold"))
```

Ahora, comparando la cantidad de proyectos por año y desglosados por temas,
podemos notar que:
...
ggplot(data = Proyectos_anp_anio, aes(x = as.character(anyo_proy), y = n, colour = as.character(anyo_proy))) +
  ...
  ggtitle("Número de proyectos realizados por dirección y año") +
  theme(
    plot.title = element_text(hjust = 0.5, face="bold")
    )
...
A este nivel, el análisis involucrará principalmente la relación entre actividades
y unidades de medida de las mismas, debido a que este punto es el esencial para
lograr una estandarización de los datos capturados, y con ello, poder efectuar
análisis que sinteticen información de todas las ANP's.
...
En la siguiente gráfica se puede observar que el 75% de las combinaciones actividad +
unidad tiene menos de 30 datos, lo que impacta en su alcance para hacer análisis estandarizados utilizando datos de varias ANP's
...
ggplot(data = Actividades_unidades_comparables, aes(x = "1", y = n)) +
  ...
  ggtitle("Número de actividades registradas\n con la misma unidad de medida") +
  theme(
...
observar que todas excepto una utilizan varias unidades de medición distintas.
...
kable(Actividades_prioritarias_numero_registros_comparables %>%
    group_by(indice) %>%
    summarise(
...
Posteriormente, nos concentramos en encontrar dentro de cada tema prioritario,
cuáles eran las actividades prioritarias (y con qué unidades), que tienen mayor
representabilidad entre ANP's, es decir, que se realizan en la mayor cantidad
de ANP's posibles.
...
Ahora, agregando por ANP, seleccionamos actividades con metas comparables y
...
kable(actividades_unidades_interes, caption = "Actividades + unidades realizadas por la mayor cantidad de ANP's")
