...
kable(Actividades_prioritarias_num_unidades)
```
...
kable(Actividades_prioritarias_numero_registros_table)
...
La siguiente tabla muestra el índice de actividades (este índice es aplicable
sólo a la gráfica anterior, puesto que ordena por "diferencia"):
...
kable(Actividades_prioritarias_numero_anps_table)
# pandoc.table(Actividades_prioritarias_numero_registros_comparables_table, split.cells = c("20%","80%"))
```

A continuación se muestran, para cada tema prioritario, las dos combinaciones de
actividad + unidad de medida que tienen más representatividad entre ANPs y
direcciones regionales:

```{r, echo = FALSE}
# Tabla de unidades de actividades prioritarias utilizadas por el mayor número de ANP's,
# Se elegirán para cada tema las dos combinaciones de actividad + unidad más usadas
# en ANPs.

Actividades_unidades_mas_representativas <- Actividades_prioritarias_unidades_direcciones %>%
...
kable(Actividades_unidades_mas_representativas)
```

La tabla anterior fue uno de los criterios de selección de las actividades y sus
unidades de medida que utilizamos para la propuesta que realizamos en esta consultoría,
sin embargo, no fue la única, puesto que también tomamos en cuenta los intereses
de directivos de la CONANP, así como la capacidad de georreferenciación de las
variables.

## Ejemplos de análisis posibles con registros comparables de cada actividad

El ultimo análisis de este documento tiene como fin principal mostrar la clase
de análisis que se pueden lograr cuando se cuentan con:

* Varios registros por actividad
* Una misma unidad de medida para todos los registros

Este corresponde a **una gráfica de metas registradas y alcanzadas contra el tiempo,
agregadas por ANP, para cada actividad.**

La actividad + unidad de medida tiene un id, que corresponde a la columna
**id_actividad_unidad** de la tabla anterior, de esta manera, se puede saber qué
gráfica corresponde a qué actividad, y en qué unidad de medida están las metas.
...
Actividades_prioritarias_unidades_realizadas_anp <- Actividad_poa %>%
  filter(!is.na(anio)) %>%
  inner_join(Actividades_unidades_mas_representativas, by = c("actividad", "unidad")) %>%
  select(
    -realizada_1,
    -realizada_2,
    -realizada_3,
...
ggplot(data = Actividades_prioritarias_unidades_realizadas_anp,
  ...
  # de scales, para que no utilice notación científica
  scale_y_continuous(labels = comma) +
  ggtitle("Metas trimestrales no acumulativas por id de actividad + unidad")
...
ggplot(data = Actividades_prioritarias_unidades_realizadas_anp,
  ...
  # de scales, para que no utilice notación científica
  scale_y_continuous(labels = comma) +
  ggtitle("Metas trimestrales acumulativas por id de actividad + unidad")
```

Como ejemplo para interpretar las gráficas anteriores, tomemos la gráfica 21,
que (observando la tabla) corresponde a la actividad
**"Elaborar y ejecutar programas de inspección y vigilancia en las ANP y sus zonas de influencia, para detectar y/o prevenir ilícitos ambientales"**
con unidad de medida
**"Hectáreas supervisadas mediante actividades de vigilancia"**

Se puede ver que esta actividad sólo se realizó en el 2016, y que en los primeros
3 trimestres, la cantidad de hectáreas supervisadas no llegó a la meta. En el
cuarto la superó con creces, sin embargo, al estudiar la gráfica acumulativa,
vemos que esto no alcanzó a compensar el déficit de los primeros 3 trimestres.

Finalmente, cabe destacar que las gráficas anteriores se pueden realizar por ANP,
utilizando una interfaz amigable (como un dashboard).
...
* Análisis general a nivel de proyectos:
    ...
    **"Comunicación ,Educación, Cultura y participación social para la conservación"**
    y **"Economía de la conservación"** son los más importantes, tanto por el número
    de ANP’s que los implementan, como por el número de proyectos que tienen.
    + No hubo un cambio radical de prioridades entre el 2015 y el 2016.
    + En general, se redujeron la cantidad de ANP's y proyectos implementados para
    cada objetivo y tema en el 2016 con respecto al 2015.
    + Los objetivos con menor importancia en cantidad de proyectos / ANP's, cuentan
    con pocos temas, pero que son de marcado interés (medido por la cantidad de
    proyectos y ANP's que los implementan).
  
* Análisis general a nivel de actividades:
    + El 75% de las combinaciones de actividad + unidad de medida tiene menos de
    30 datos, sin embargo, el 25% de las actividades restantes concentra casi el
    70% de los datos.
    + Conclusiones de este tipo se pueden obtener al estudiar la gráfica de
    "**% de registros y actividades + unidades en la base que satisfacen un requerimiento mínimo de comparabilidad**"
    + El número de registros de determinada actividad + unidad de medida en cierto
    año es prácticamente el número de ANP’s que la implementaron en dicho año, por
    lo que mayor cantidad de registros en un año se traduce en mayor cantidad de
    ANP's que implementan cierta actividad de esta manera.
    
* Análisis específico a nivel de actividades prioritarias:
    + En promedio, si las unidades fueran estandarizadas, se tendrían casi 60
    registros comparables más por actividad.
    + En promedio, si las unidades fueran estandarizadas, se podrían comparar más
    de 13 ANPs y direcciones regionales adicionales por actividad.
    + La discrepancia de este resultado con el último de la sección anterior
    posiblemente se debe a que:
        + Se están considerando dos años, no sólo uno, y varias ANP's pueden realizar
        la actividad dos veces en ese lapso.
        + Varias ANP's registran la misma actividad, en el mismo año y con distintas
        unidades de medida, por lo que al estandarizar unidades no se obtendrían
        nuevas ANP's.
    + Al tener para cada actividad registros estandarizados en unidades de medida,
    se pueden acumular mayor cantidad de registros comparables, que pueden dar lugar
    a poderosos análisis.
